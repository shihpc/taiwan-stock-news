...
不可直接相除比對。
**這句警告放在可見文字、不是 tooltip（2026-08-30 修）**：上線首版把「基準不同、不可直接相除」
只寫進 `title=`，手機沒有 hover 就永遠看不到，而看到兩個數字對不起來的正是手機讀者。
現在殖利率下方固定一行小字 `dyCap`（`trackDividendRow`）寫明口徑與這句警告，
`dyTip`（`title=`）保留更完整的來源說明作桌機補充；資料日改掛在殖利率數字後面。
誠實原則：只陳述數字與口徑，不加評語、不上漲跌色。取不到 `dy` 時只少這一欄，股利列其餘照顯；
只有 `dy`、沒有股利資料時該列仍會出現（兩者皆無才整列不畫）。
